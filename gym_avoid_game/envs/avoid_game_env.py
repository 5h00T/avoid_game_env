...
from gym_avoid_game.envs.resource.task import task1, task2, task3
...
class v0AvoidGameTask3Env(gym.Env):
    def __init__(self):
        self.screen = None
        self.action_space = gym.spaces.Discrete(9)
        self.observation_space = gym.spaces.Box(low=0, high=255, shape=(200, 200, 3), dtype=np.uint8)
    ...
    def reset(self):
        self.done = False
        pygame.init()
        self.task_manager = task_manager.TaskManager(task3.Task3)
        try:
            self.screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
        except:
            os.environ["SDL_VIDEODRIVER"] = "dummy"
...
class v1AvoidGameTask3Env(gym.Env):
    def __init__(self):
        self.screen = None
        self.action_space = gym.spaces.Discrete(3)
        self.observation_space = gym.spaces.Box(low=0, high=255, shape=(200, 200, 3), dtype=np.uint8)
